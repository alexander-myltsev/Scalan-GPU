...
object PArraysSeqTests {
  val samples = new ScalanSequential with DslSamples {
  ...
  def myTest = {
    val sparseVec = Array((0, 10f), (2, 30f), (3, 40f))
    val sparseVecPA = fromArray(sparseVec)
    val vec = Array(10f, 20f, 30f, 40f)
    val vecPA = fromArray(vec)
    val res = sparseVectorMul(sparseVecPA, vecPA)
    println(res)
  }
